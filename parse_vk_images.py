...
# Например, чтобы не вводили кириллицей ID
# И вообще надо написать функцию, которая на вход принимает ссылку на группу в enter_the_vk_groups отдаёт только id
# если нет папки images, то ее надо создать
# Желательно формата images_<#_группы>
...
def enter_the_vk_groups(all_vk_groups):
    ...
    """На вход принимается ссылка на группу, а отдаётся только id"""
    vk_group = input('\nEnter the group ID:\n').split('https://vk.com/')[1] # здесь отсекается часть zaokskypr
    try:
        vk_group = vk_group.split('public')[1] # здесь отсекается часть public у public153967240 при наличии слова public
        vk_group = int(vk_group)
    except:
        pass
...
def transform_to_df(photo_request):
    ...
    for item in photo_request:
        srcs = np.append(srcs, item['src_big']) # src - это прямая ссылка на фото. ЕСТЬ: src, src_big, src_small, src_xbig, src_xxxbig
